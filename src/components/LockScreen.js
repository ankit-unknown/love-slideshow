    ...
    <div className={`ls-overlay${unlocking ? ' ls-unlocking' : ''}`}>
      ...
      <div className={`ls-dots${shaking ? ' ls-shake' : ''}`}>
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className={`ls-dot${i < code.length ? ' ls-dot-filled' : ''}`} />
        ))}
      </div>
      ...
      <div className="ls-keypad">
        {KEYS.map((k, i) => (
          <button
            key={i}
            className={`ls-key${k === '' ? ' ls-key-empty' : ''}${k === 'del' ? ' ls-key-del' : ''}`}
            onClick={() => press(k)}
      ...
      <p className={`ls-msg${status === 'wrong' ? ' ls-msg-error' : ''}${status === 'welcome' ? ' ls-msg-success' : ''}`}>
        {status === 'wrong' ? 'Try again, love ♥' : status === 'welcome' ? 'Welcome ♥' : 'Enter 6-digit code'}
      </p>
    </div>
  );
}